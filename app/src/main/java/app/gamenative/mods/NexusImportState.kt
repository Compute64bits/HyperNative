...
internal object NexusImportState {
    ...
    private fun apiMessage(
        ...
        val base = when (error.reason) {
            ...
            NexusApiErrorReason.DOWNLOAD_AUTHORIZATION_REQUIRED ->
                "Free Nexus accounts must authorize this file on the Nexus Mods website before HyperNative can download it."
            NexusApiErrorReason.DOWNLOAD_AUTHORIZATION_INVALID ->
                "Nexus rejected the website download authorization. Make sure the browser and HyperNative use the same Nexus account, then try again."
            NexusApiErrorReason.DOWNLOAD_AUTHORIZATION_EXPIRED ->
