    ...
    suspend fun checkInstallHealthForApp(
        ...
        installs.forEach { install ->
            ...
            if (install.status == ModInstallStatus.APPLIED.name) {
                if (recipes.none { it.enabled }) {
                    add(ModHealthSeverity.ERROR, "Applied mod has no placement recipe", "HyperNative cannot verify or safely remove deployed files.", install)
        ...
        if (unsafeBackupCount > 0) {
            add(
                ...
                "$unsafeBackupCount backup record(s) point outside HyperNative's backup folder.\n${unsafeBackupExamples.joinToString("\n")}",
            )
        }
